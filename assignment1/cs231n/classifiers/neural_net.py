...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    log_scores = -np.log(exp_scores)
    loss = (np.sum(log_scores[range(N),y],keepdims=True))/N + (0.5 * reg * np.sum(W1**2)) + (0.5 * reg * np.sum(W2**2)) 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    dscores = np.zeros_like(exp_scores)
    dscores[range(N),y] = 1
    dscores = (-dscores + exp_scores)/N
    dW2 = np.dot(hiddenLayer.T,dscores) + (reg * W2)
